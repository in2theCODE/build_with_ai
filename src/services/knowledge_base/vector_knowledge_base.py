...
try:
    ...
    logging.warning(
        "Vector DB dependencies not installed. Run: pip install sentence-transformers qdrant-client"
    )
                ...
                self.client.upsert(
                    ...
                    points=[
                        models.PointStruct(
                            ...
                            payload={"key": key, "code": code, "metadata": metadata},
                        )
                    ],
                )
            ...
            if self.storage_type == "qdrant":
                ...
                if data_path.exists():
                    with open(data_path, "r") as f:
                        data = json.load(f)
                        return {"code": data.get("code", ""), "metadata": data.get("metadata", {})}
                return None
