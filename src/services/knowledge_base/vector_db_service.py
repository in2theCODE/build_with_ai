...
class VectorDBService(BaseComponent):
    ...
    def _initialize_vector_db(self):
        ...
        if vector_db_type == "milvus":
            ...
            self.logger.info(
                f"Connected to Milvus at {connection_params.get('host', 'localhost')}:{connection_params.get('port', 19530)}"
            )
            ...
            self.logger.info(
                f"Connected to Qdrant at {connection_params.get('host', 'localhost')}:{connection_params.get('port', 6333)}"
            )
            ...
            self.logger.warning(
                f"Unknown vector DB type: {vector_db_type}, using in-memory fallback"
            )
    ...
    async def _handle_vector_db_event(self, event):
        ...
        if not collection:
            await self._emit_error_response(operation, correlation_id, "Missing collection name")
            return
    ...
    async def _handle_query(self, event, collection, correlation_id):
        ...
        if not vector:
            await self._emit_error_response("query", correlation_id, "Missing vector for query")
            return
    ...
    async def _handle_store(self, event, collection, correlation_id):
        ...
        start_time = time.time()
        result = await self.vector_db_client.store_vector(collection, vector, metadata, id=id)
