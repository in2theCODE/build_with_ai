...
class SpecRegistryEventAdapter(BaseComponent):
    ...
    async def _handle_spec_instance_updated(self, event: BaseEvent):
        ...
        try:
            ...
            # For now, we'll just log this (spec instances are handled differently)
            self.logger.info(
                f"Received spec instance update: {instance_id} for spec {spec_sheet_id}"
            )
    ...
    async def _handle_spec_instance_completed(self, event: BaseEvent):
        ...
        try:
            ...
            self.logger.info(
                f"Marked spec sheet as completed via instance: {spec_sheet_id}"
            )
    ...
    async def _handle_spec_sheet_analysis_completed(self, event: BaseEvent):
        ...
        try:
            self.logger.info(
                f"Handling spec sheet analysis completion: {event.event_id}"
            )
            ...
            self.logger.info(
                f"Updated spec sheet with analysis results: {spec_sheet_id}"
            )
    ...
    async def _handle_spec_sheet_evolution_suggested(self, event: BaseEvent):
        ...
        try:
            self.logger.info(
                f"Handling spec sheet evolution suggestion: {event.event_id}"
            )
            ...
            self.logger.info(
                f"Updated spec sheet with evolution suggestions: {spec_sheet_id}"
            )
    ...
    async def _handle_spec_sheet_evolution_applied(self, event: BaseEvent):
        ...
        try:
            self.logger.info(
                f"Handling spec sheet evolution application: {event.event_id}"
            )
            ...
            self.logger.info(
                f"Updated spec sheet with applied evolution: {spec_sheet_id}"
            )
    ...
    async def create_spec(
        self, spec_type: str, project_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def update_spec(
        self, spec_id: str, field_updates: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if self.enable_events and hasattr(self, "event_emitter"):
            ...
            event = BaseEvent(
                ...
                source_container=Components.SYNTHESIS_ENGINE,
                payload={"spec_sheet_id": spec_id, "field_updates": field_updates},
            )
    ...
    async def list_specs(
        self, project_id: Optional[str] = None, spec_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
