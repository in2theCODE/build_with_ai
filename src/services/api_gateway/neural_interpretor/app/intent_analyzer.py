...
class IntentAnalyzer:
    def __init__(self, config: Dict[str, Any], embedding_client: EmbeddingClient, pattern_matcher: PatternMatcher):
        self.config = config
    ...
    async def analyze_query(self, query: str, system_message: Optional[str] = None, require_delegation: bool = False) -> QueryIntent:
        """
        ...
        elif fast_path_match and fast_path_match["score"] > self.config["intent_analysis"].get("fast_path_confidence_threshold", 0.85):
            processing_mode = ProcessingMode.REACTIVE
            logger.info(f"Query {query[:50]}... matched pattern {fast_path_match['id']} with score {fast_path_match['score']}")
        else:
            # Use complexity scoring
            processing_mode = ProcessingMode.DELIBERATIVE if complexity_score > self.complexity_threshold else ProcessingMode.REACTIVE
            logger.info(f"Query {query[:50]}... assigned to {processing_mode} mode with complexity {complexity_score:.2f}")
    ...
    def _count_complexity_keywords(self, text: str) -> int:
        ...
        keywords = [
            ...
            "synthesize", "interpret", "infer", "deduce", "hypothesize",
    ...
    def _detect_conversational_markers(self, text: str) -> int:
        ...
        markers = ["please", "could you", "would you", "I want", "I need", "help me", "can you", "tell me", "show me"]
        count = 0
        text_lower = text.lower()
    ...
    def _calculate_complexity_score(self, signals: Dict[str, Any]) -> float:
        ...
        norm_token_count = min(signals["token_count"] / self.config["intent_analysis"].get("max_token_threshold", 200), 1.0)
