...
from src.services.shared.events.base_event import EventType
...
async def _handle_event(self, event: BaseEvent):
    ...
    """
    self.logger.debug(
        f"Handling event: {event.event_type} with correlation_id: {event.correlation_id}"
    )
    ...
    self.pending_requests[correlation_id]["responses"].append(
        {"event_type": event.event_type, "data": event.data, "timestamp": time.time()}
    )
    ...
    if "primary" in self.db_connections:
        try:
            ...
            await db.results.update_one(
                {"correlation_id": correlation_id}, {"$set": result}, upsert=True
            )
