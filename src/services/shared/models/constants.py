...
class Paths(BaseModel):
    ...
    # Core services paths
    AGGREGATORS_PATH: str = Field(
        "build_with_ai/src/services/aggregators", description="Aggregators path"
    )
    API_GATEWAY_PATH: str = Field(
        "build_with_ai/src/services/api_gateway", description="API Gateway path"
    )
    AST_CODE_GENERATOR_PATH: str = Field(
        "build_with_ai/src/services/ast_code_generator", description="AST code generator path"
    )
    CONSTRAINT_RELAXER_PATH: str = Field(
        "build_with_ai/src/services/constraint_relaxer", description="Constraint relaxer path"
    )
    FEEDBACK_COLLECTOR_PATH: str = Field(
        "build_with_ai/src/services/feedback_collector", description="Feedback collector path"
    )
    GRAFANA_PATH: str = Field("build_with_ai/src/services/grafana", description="Grafana path")
    INCREMENTAL_SYNTHESIS_PATH: str = Field(
        "build_with_ai/src/services/incremental_synthesis", description="Incremental synthesis path"
    )
    KNOWLEDGE_BASE_PATH: str = Field(
        "build_with_ai/src/services/knowledge_base", description="Knowledge base path"
    )
    LANGUAGE_INTEROP_PATH: str = Field(
        "build_with_ai/src/services/language_interop", description="Language interoperability path"
    )
    LLM_VOLUME_PATH: str = Field(
        "build_with_ai/src/services/llm_volume", description="LLM volume path"
    )
    META_LEARNER_PATH: str = Field(
        "build_with_ai/src/services/meta_learner", description="Meta learner path"
    )
    NEURAL_CODE_GENERATOR_PATH: str = Field(
        "build_with_ai/src/services/neural_code_generator", description="Neural code generator path"
    )
    PROJECT_MANAGER_PATH: str = Field(
        "build_with_ai/src/services/project_manager", description="Project manager path"
    )
    PROMETHEUS_PATH: str = Field(
        "build_with_ai/src/services/prometheus", description="Prometheus path"
    )
    SHARED_PATH: str = Field("build_with_ai/src/services/shared", description="Shared modules path")
    SPEC_INFERENCE_PATH: str = Field(
        "build_with_ai/src/services/spec_inference", description="Spec inference path"
    )
    SPEC_REGISTRY_PATH: str = Field(
        "build_with_ai/src/services/spec_registry", description="Spec registry path"
    )
    SPECIFICATION_PARSER_PATH: str = Field(
        "build_with_ai/src/services/specification_parser", description="Specification parser path"
    )
    SYNTHESIS_ENGINE_PATH: str = Field(
        "build_with_ai/src/services/synthesis_engine", description="Synthesis engine path"
    )
    TEMPLATE_DISCOVERY_PATH: str = Field(
        "build_with_ai/src/services/template_discovery", description="Template discovery path"
    )
    TEMPLATE_LIB_VOLUME_PATH: str = Field(
        "build_with_ai/src/services/template_lib_volume", description="Template library volume path"
    )
    VERIFIER_PATH: str = Field("build_with_ai/src/services/verifier", description="Verifier path")
    VERSION_MANAGER_PATH: str = Field(
        "build_with_ai/src/services/version_manager", description="Version manager path"
    )
    WORKFLOW_ORCHESTRATOR_PATH: str = Field(
        "build_with_ai/src/services/workflow_orchestrator", description="Workflow orchestrator path"
    )
    WORKFLOW_REGISTRY_PATH: str = Field(
        "build_with_ai/src/services/workflow_registry", description="Workflow registry path"
    )

    # Configuration paths
    CONFIGS_PATH: str = Field(
        "build_with_ai/src/services/configs", description="Configuration files path"
    )

    # Model paths (absolute)
    DEFAULT_MODEL_PATH: str = Field(
        "/Users/justinrussell/.models/deepseek-coder-6.7b-base",
        description="Default path for code generation model",
    )
    DEFAULT_INFERENCE_MODEL_PATH: str = Field(
        "/Users/justinrussell/.models/mistral-mlx", description="Path for neural interpreter model"
    )
...
class ModelConfig(BaseModel):
    ...
    DEFAULT_MODEL_PATH: str = Field(
        "/Users/justinrussell/.models/deepseek-coder-6.7b-base", description="Default model path"
    )
    DEFAULT_INFERENCE_MODEL_NAME: str = Field(
        "Mistral MLX", description="Default inference model name"
    )
    DEFAULT_INFERENCE_MODEL_PATH: str = Field(
        "/Users/justinrussell/.models/mistral-mlx", description="Path for neural interpreter model"
    )
