"""
Specification sheet models for code generation requirements.

This module defines the data models for specification sheets, which are
templates that describe what code should be generated. Specification sheets
provide a structured way to capture requirements for code generation tasks.

Classes:
    FieldDefinition: Field definition in a spec sheet
    SectionDefinition: Section definition in a spec sheet
    SpecSheetDefinition: Template for a specification sheet
    FieldValue: Field value in a completed spec sheet
    SectionValues: Section values in a completed spec sheet
    SpecSheet: Completed specification sheet
    SpecSheetGenerationRequestMessage: Message requesting spec sheet generation
    SpecSheetCompletionRequestMessage: Message requesting AI completion of a spec sheet
    SpecSheetDefinitionRequest: Request for a spec sheet definition
    SpecSheetDefinitionResponse: Response with the best matching spec sheet definition
"""
...
class SpecSheetDefinitionRequest(BaseMessage):
    ...
    additional_criteria: Dict[str, Any] = Field(
        default_factory=dict, description="Additional criteria"
    )
    created_at: str = Field(
        default_factory=lambda: datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        description="Creation timestamp",
...
class SpecSheetDefinitionResponse(BaseMessage):
    ...
    request_id: str = Field(..., description="Request identifier")
    spec_sheet_definition_id: Optional[str] = Field(
        None, description="Spec sheet definition identifier"
    )
    spec_sheet_definition_path: Optional[str] = Field(
        None, description="Spec sheet definition path"
    )
    match_confidence: float = Field(0.0, description="Match confidence")
    spec_sheet_definition_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Spec sheet definition metadata"
    )
