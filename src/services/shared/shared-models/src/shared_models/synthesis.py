"""
Code synthesis app and result representation.

This module defines app related to code synthesis, which is the process
...
# app/services/shared/app/synthesis.py
...
class SynthesisResult(BaseModel):
    ...
    code: Optional[str] = Field(default=None, description="String representation of the generated code")
    # No alias here - we'll handle backwards compatibility differently
    confidence_score: float = Field(default=0.0, description="Confidence score of the synthesis result (0.0-1.0)")
    time_taken: float = Field(default=0.0, description="Time taken for synthesis in seconds")

    # Strategy and metadata
    strategy: Optional[SynthesisStrategyType] = Field(default=None, description="Strategy used for synthesis")
    ...
    errors: List[str] = Field(default_factory=list, description="List of errors encountered during synthesis")
    ...
    verification_result: Optional[Any] = Field(default=None, description="Verification result if available")
