...
# Use standard logging instead of LoggerService directly
logger = logging.getLogger(__name__)

# Type variable for flexibility
T = TypeVar("T")
...
class SchemaRegistryClient:
    ...
    def register_schema(self, subject: str, avro_schema: Dict[str, Any]) -> int:
        ...
        try:
            response = requests.post(f"{self.url}/subjects/{subject}/versions", headers=self.headers, data=payload)
    ...
    def register_avro_base_model(self, model_class: Type[T], subject: Optional[str] = None) -> int:
        """
        Register an AvroBase model with the schema registry.

        Args:
            model_class: Class (not instance) that has avro_schema method
            subject: Optional subject name, defaults to class name

        Returns:
            Schema ID from registry
        """
        # If subject not provided, get it from the model if available
        if subject is None and hasattr(model_class, "get_schema_subject"):
            subject = model_class.get_schema_subject()
        elif subject is None:
            # Default subject naming
            subject = f"{model_class.__name__}"

        # Check if the model class has the avro_schema method
        if not hasattr(model_class, "avro_schema"):
            raise AttributeError(
                f"Model class {model_class.__name__} doesn't have an 'avro_schema' method. "
                f"Make sure it inherits from AvroBase or AvroBaseModel."
            )

        # Get Avro schema from the model
        avro_schema = model_class.avro_schema()

        # Register schema
        schema_id = self.register_schema(subject, avro_schema)
    ...
    def get_model_schema_id(self, model_class: Type[T]) -> Optional[int]:
        """Get the schema ID for a model class."""
        return getattr(model_class, "__avro_schema_id__", None)

    def get_model_schema_subject(self, model_class: Type[T]) -> Optional[str]:
    ...
    def get_schema_id(self, subject: str) -> Optional[int]:
        ...
        try:
            response = requests.get(f"{self.url}/subjects/{subject}/versions/latest", headers=self.headers)
            response.raise_for_status()
            schema_id = response.json()["id"]
...
def map_pydantic_type_to_avro(field_type: str, field_schema: Dict[str, Any]) -> Any:
    ...
    if field_type == "object":
        ...
        for prop_name, prop_schema in properties.items():
            ...
            field_def = {"name": prop_name}

            if prop_name not in required:
                field_def["type"] = ["null", map_pydantic_type_to_avro(prop_type, prop_schema)]
                field_def["default"] = None
            else:
                field_def["type"] = map_pydantic_type_to_avro(prop_type, prop_schema)
