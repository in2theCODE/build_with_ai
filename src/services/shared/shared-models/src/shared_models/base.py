"""
Base app for Apache Pulsar integration with Avro schema serialization.

This module provides the foundation for all app in the system, with full
support for Pydantic v2, Fast Avro serialization, and Apache Pulsar schema
registry integration. The app in this module serve as base classes that
other app should inherit from to ensure consistent serialization and
schema generation.
...
class AvroBaseModel(AvroBase):
    """Base model with Avro and Pulsar support for all app in the system.
    ...
    model_config = ConfigDict(frozen=True, json_encoders={datetime: lambda dt: dt.isoformat().replace("+00:00", "Z")})
