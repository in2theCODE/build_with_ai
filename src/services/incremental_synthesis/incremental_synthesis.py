...
from src.services.shared.models import FormalSpecification
from src.services.shared.models import SynthesisResult
import z3
...
class IncrementalSynthesis(BaseComponent):
    ...
    def _extract_variables_from_constraint(self, constraint: Any) -> Set[str]:
        ...
        if hasattr(constraint, "children"):
            ...
            while queue:
                ...
                if (
                    z3.is_const(node)
                    and not z3.is_bool(node)
                    and not z3.is_int_value(node)
                    and not z3.is_real_value(node)
                ):
    ...
    def _create_component_spec(
        self, formal_spec: FormalSpecification, component_vars: Set[str]
        ...
        if "parameters" in component_ast:
            component_ast["parameters"] = [
                param
                for param in component_ast["parameters"]
    ...
    def _combine_conditional(self, component_results: List[SynthesisResult]) -> SynthesisResult:
        ...
        main_body.append({"type": "return", "value": {"type": "variable", "name": "result"}})
    ...
    def _get_function_arguments(self, ast: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if "parameters" in ast and isinstance(ast["parameters"], list):
            for param in ast["parameters"]:
                if isinstance(param, dict) and "name" in param:
                    args.append({"type": "variable", "name": param["name"]})
                elif isinstance(param, str):
                    args.append({"type": "variable", "name": param})
