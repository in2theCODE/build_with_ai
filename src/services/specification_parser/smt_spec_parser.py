...
class SMTSpecificationParser(BaseComponent):
    ...
    def __init__(self, **params):
        ...
        self._param_pattern = re.compile(r'(?:parameter|param|argument|arg|input|takes|accepts)\s+(\w+)', re.IGNORECASE)
        self._type_patterns = {
            'int': re.compile(r'(?:integer|int)\s+(\w+)|(\w+)\s+(?:is|as|:)\s+(?:an?\s+)?(?:integer|int)', re.IGNORECASE),
            'float': re.compile(r'(?:float|decimal|double)\s+(\w+)|(\w+)\s+(?:is|as|:)\s+(?:a\s+)?(?:float|decimal|double)', re.IGNORECASE),
            'bool': re.compile(r'(?:boolean|bool)\s+(\w+)|(\w+)\s+(?:is|as|:)\s+(?:a\s+)?(?:boolean|bool)', re.IGNORECASE),
            'str': re.compile(r'(?:string|text|str)\s+(\w+)|(\w+)\s+(?:is|as|:)\s+(?:a\s+)?(?:string|text|str)', re.IGNORECASE),
            'array': re.compile(r'(?:array|list|sequence)\s+(?:of\s+)?(\w+)\s+(\w+)|(\w+)\s+(?:is|as|:)\s+(?:an?\s+)?(?:array|list|sequence)\s+(?:of\s+)?(\w+)', re.IGNORECASE),
            'set': re.compile(r'(?:set)\s+(?:of\s+)?(\w+)\s+(\w+)|(\w+)\s+(?:is|as|:)\s+(?:a\s+)?(?:set)\s+(?:of\s+)?(\w+)', re.IGNORECASE),
            'enum': re.compile(r'(?:enum|enumeration)\s+(\w+)|(\w+)\s+(?:is|as|:)\s+(?:an?\s+)?(?:enum|enumeration)', re.IGNORECASE),
        }
        ...
        self._comparison_pattern = re.compile(r'([\w_]+)\s*(>|<|==|=|!=|<=|>=|contains|in|not\s+in|is\s+not)\s*([\w_]+|\d+(?:\.\d+)?|"[^"]*"|\'[^\']*\')', re.IGNORECASE)

        # Enhanced patterns for ranges and collections
        self._range_pattern = re.compile(r'([\w_]+)\s+(?:in range|between)\s+(\d+(?:\.\d+)?)\s+(?:and|to)\s+(\d+(?:\.\d+)?)', re.IGNORECASE)
        self._collection_pattern = re.compile(r'([\w_]+)\s+(?:in|is in|is one of|belongs to|member of)\s+\[(.*?)\]', re.IGNORECASE)

        # Key-value pattern for tokenization
        self._key_value_pattern = re.compile(r'([\w_]+)\s*[:=]\s*([\w_]+|\d+(?:\.\d+)?|"[^"]*"|\'[^\']*\')', re.IGNORECASE)

        # Example patterns
        self._example_pattern = re.compile(r'(?:example|instance|test case).*?(?:input|in)(?:\s*:)?\s*(.*?)(?:output|out|result)(?:\s*:)?\s*(.*)', re.IGNORECASE | re.DOTALL)
        self._example_kv_pattern = re.compile(r'([\w_]+)\s*[:=]\s*([\w_]+|\d+(?:\.\d+)?|"[^"]*"|\'[^\']*\')', re.IGNORECASE)

        # Type inference patterns
        self._result_type_pattern = re.compile(r'return(?:s|ing)?\s+(?:a|an)?\s+(int(?:eger)?|float|double|decimal|bool(?:ean)?|str(?:ing)?|text|array|list|set|enum(?:eration)?|dict(?:ionary)?|map)', re.IGNORECASE)
    ...
    def _extract_parameter_names(self, specification: str) -> List[str]:
        ...
        def _create_ast(self, specification: str, context: Dict[str, Any],
                        parameter_names: List[str], types: Dict[str, str]) -> Dict[str, Any]:
    ...
    def _infer_types(self, specification: str, context: Optional[Dict[str, Any]],
                     parameter_names: List[str]) -> Dict[str, str]:
        ...
        if result_match:
            ...
            if 'int' in result_type_text or 'integer' in result_type_text:
                types["result"] = "int"
            elif 'float' in result_type_text or 'double' in result_type_text or 'decimal' in result_type_text or 'real' in result_type_text:
                types["result"] = "float"
    ...
    def _extract_constraints(self, specification: str, context: Dict[str, Any],
                             parameter_names: List[str], types: Dict[str, str]) -> List[Any]:
    ...
    def _create_comparison_constraint(self, z3_var: Any, operator: str,
                                      right_val: str, var_type: str) -> Optional[Any]:
    ...
    def _parse_constraint_string(self, constraint_str: str, z3_vars: Dict[str, Any],
                                 types: Dict[str, str]) -> Optional[Any]:
    ...
    def _extract_examples(self, specification: str, context: Dict[str, Any],
                          parameter_names: List[str], types: Dict[str, str]) -> List[Dict[str, Any]]:
        ...
        for i, line in enumerate(lines):
            if re.search(r'\b(example|instance|test case|sample input|test input)\b', line, re.IGNORECASE):
    ...
    def _parse_example_text(self, input_text: str, output_text: str,
                            parameter_names: List[str], types: Dict[str, str]) -> Optional[Dict[str, Any]]:
