...
from .core.intent_analyzer import IntentAnalyzer, ProcessingMode
...
class AppState:
    ...
    async def initialize(self):
        ...
        self.embedding_client = EmbeddingClient(
            self.config["embedding_service"]["url"]
        )
...
async def get_intent_analyzer():
    if app_state.intent_analyzer is None:
        raise HTTPException(
            status_code=503,
            detail="Service initializing, please try again shortly"
        )
...
async def health_check():
    ...
    return HealthResponse(
        status="healthy" if neo4j_status == "healthy" and embedding_status == "healthy" else "degraded",
        version="1.0.0",
...
async def interpret_query(
    ...
    try:
        ...
        intent_analysis = IntentAnalysis(
            ...
            confidence=1.0 if intent.processing_mode == ProcessingMode.DELIBERATIVE else 
                      (intent.fast_path_match["score"] if intent.fast_path_match else 0.7),
...
async def generic_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content={"error": str(exc), "error_code": "INTERNAL_ERROR"}
    )
